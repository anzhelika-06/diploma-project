  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        u.email,
        u.avatar_emoji,
        COALESCE(u.carbon_saved, 0) as carbon_saved,
        u.created_at,
        ROW_NUMBER() OVER (ORDER BY COALESCE(u.carbon_saved, 0) DESC, u.created_at ASC) as rank
      FROM users u
      ORDER BY COALESCE(u.carbon_saved, 0) DESC, u.created_at ASC
      LIMIT $1 OFFSET $2
    `;
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        t.description,
        t.avatar_emoji,
        COALESCE(t.carbon_saved, 0) as total_carbon_saved,
        t.member_count,
        t.created_at,
        ROW_NUMBER() OVER (ORDER BY COALESCE(t.carbon_saved, 0) DESC, t.created_at ASC) as rank
      FROM teams t
      ORDER BY COALESCE(t.carbon_saved, 0) DESC, t.created_at ASC
      LIMIT $1 OFFSET $2
    `;
    
    const result = await executeQueryWithLogging(pool, query, [pagination.limit, pagination.offset]);
    
    // Получаем общее количество всех команд
    const countQuery = `
      SELECT COUNT(*) as total
      FROM teams
    `;
